...
module.exports = function(req, _server, _app){
	console.log("appLogs.js","ORIGINAL REQ", req.body);
	try{
		...
		opts.session = [
	        ...
	        { op: 'exec', command: 'export SERVER_ID='+_server._id+' CALLBACK_TOKEN='+req.techpool.user.access_token+' CALLBACK_URL='+AppConfig.AppDomain+'/v1/server/events ACTION=deploy_logs APP_NAME='+_app.app_name+' APP_ID='+_app._id+' && /home/'+_server.superuser+'/_setup.sh > _query.log.out 2>&1' },
	    ];
	    console.log("calling sshclient with", opts);
