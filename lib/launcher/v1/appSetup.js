...
var addSetup =  function(req, _server, _app, redo, retryAttempt = 1){
	...
	try{
		...
		opts.session = [
	        ...
	        { op: 'exec', canFail: false, command: 'export USERCARGOSPACEPUBKEY="'+req.techpool.credentials.custom_public_key+'" REPO_PROJECT_NAME='+repoProject+' REPO_USER='+repoUser+' SERVER_NAME='+_server.server_name+' USER_OAUTH_TOKEN='+gitToken+' REPO_VISIBILITY='+visibility+' GIT_PROVIDER='+req.body.git_provider+' SERVER_ID='+_server._id+' CALLBACK_TOKEN='+req.techpool.user.access_token+' CALLBACK_URL='+AppConfig.AppDomain+'/v1/server/events ACTION=add_app SERVER_ENTRY_POINT='+req.body.entry_point+' APP_NAME='+_app.app_name+' APP_ID='+_app._id+' PORT='+ (3000 + parseInt(_server._app_counter))+' NODE_VERSION='+_app.template_variation+' REPOSITORY="'+_app.app_repository+'" && /home/'+_server.superuser+'/.setup.sh >.app_'+_app.app_name+'.log.out 2>&1' },
	    ];
		...
		sshclient.session(opts, _x(function(err, response){
			...
			if(err && !redo){
				if(retryAttempt == 5){
					rollback(req, _app);
					try{
						req.io.to(_app.uid).emit('CREATE_APP_FAILED', {
							action: 'CREATE_APP_FAILED',
							data: _app,
							message: 'Create app failed after 5 retry attempts'
						});
					}catch(err){
			...
			try{
				req.io.to(_app.uid).emit('CREATE_APP_SUCCESS', {
					action: 'CREATE_APP_SUCCESS',
					data: _app,
