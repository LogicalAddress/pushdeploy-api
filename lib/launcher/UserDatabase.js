...
var UserDatabaseModel = require('../../models/UserDatabase');
...
UserDatabase.prototype.findAll = function(queryParams){
	return new Promise(function(resolve, reject) {
		UserDatabaseModel.find(queryParams).populate('server').exec().then(function(response){
			resolve(response);
		}).catch(function(err) {
			reject(err);
		});
