...
var UserAuthCredentialModel = require('../../models/mongo/UserAuthCredentials');
/*
...
UserAuthCredential.prototype.update = function(data){
	return new Promise(function(resolve, reject) {
		UserAuthCredentialModel.findOneAndUpdate(data.query, data.update, {new : true }).exec().then(function(userCredential) {
			resolve(userCredential);
		}).catch(function(err) {
			reject(err);
		});
