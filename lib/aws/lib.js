...
function _createInstance(ec2, payload, credentials, Server, req){
	return new Promise((resolve, reject) =>{
		...
		ec2.runInstances({
			...
			if(err) { 
				process.emit('aws_instance_create_failure', Server, credentials, req);
				return reject(err);
			}
...
Aws.prototype.createInstance = function(payload, mUserAuthCrendential, Server, req){
	AwsConf.ec2.accessKeyId = payload.accessKeyId;
	AwsConf.ec2.secretAccessKey = payload.secretAccessKey;
	AwsConf.ec2.region = payload.region || 'us-west-2';
	...
	// ec2.config.update({region: payload.region || 'us-west-2'});
	return new Promise(function(resolve, reject) {
		// UserAuthCredential._findByUID(payload.uid).then((mUserAuthCrendential) => {
			...
			if(mUserAuthCrendential.aws_KeyFingerprint && mUserAuthCrendential.aws_KeyName){
				...
				_createInstance(ec2, payload, mUserAuthCrendential, req).then((ciResponse)=>{
					...
					resolve(bigPayload);
		            process.emit('aws_instance_created', bigPayload, Server, req);
					return;
				}).catch((error)=>{
					process.emit('aws_instance_create_failure', Server, mUserAuthCrendential, req);
					return reject(error);	
				});
			}else{
				...
				ec2.describeVpcs((err, dvpcsData) => {
					...
					if(err){ 
						console.log("Emited Error: aws_instance_create_failure");
						process.emit('aws_instance_create_failure', Server, null, req);
						return reject(err);
					}
