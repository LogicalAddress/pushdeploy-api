...
DNS.prototype.countZone = function(query){
	return DNSModel.count(query);
}
...
DNS.prototype.findAllZone = function(queryParams){
	return new Promise(function(resolve, reject) {
		DNSModel.find(queryParams).populate('app').lean().exec().then(function(response){
			resolve(response);
		}).catch(function(err) {
			reject(err);
		});
...
DNS.prototype.findById = function(id){
	return new Promise(function(resolve, reject) {
		DNSModel.findById(id).populate('app').lean().exec().then(function(response) {
			resolve(response);
		}).catch(function(err) {
			reject(err);
		});
...
DNS.prototype.findOneZone = function(query){
	return new Promise((resolve, reject)=>{
		DNSModel.findOne(query).populate('app').lean().exec().then((response)=>{
			resolve(response);
		}).catch(function(err) {
			reject(null);
		});
...
DNS.prototype.findAllDNSRecords = function(queryParams){
	return new Promise(function(resolve, reject) {
		DNSRecordsModel.find(queryParams).populate('zone').lean().exec().then(function(response){
			resolve(response);
		}).catch(function(err) {
			reject(err);
		});
