			...
			if(response){
				try{
					...
					opts.session = [
						...
						{ op: 'exec', command: 'export EMAIL="'+req.techpool.user.email+'" CERT_TYPE=letsencrypt TOGGLE_SSL='+ (_app.ssl_enabled ? 'on' : 'on') +' SERVER_NAME='+_app.server.server_name+' SERVER_ID='+_app.server._id+' TEMPLATE='+_app.template+' CALLBACK_URL='+AppConfig.AppDomain+'/v1/events ACTION=toggle_ssl APP_NAME='+_app.app_name+' APP_ID='+_app._id+' && /home/'+_app.server.superuser+'/.setup.sh >.app_'+_app.app_name+'.log.out 2>&1' },
				    ];
