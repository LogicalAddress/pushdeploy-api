		...
		UserApp.findAllBy({
			...
			// if(req.body.repository){
				try{
					...
					opts.session = [
						{ op: 'exec', command: 'export REPO_PROJECT_NAME='+repoProject+' REPO_USER='+repoUser+' SERVER_NAME='+_app.server.server_name+' USER_OAUTH_TOKEN='+gitToken+' REPO_VISIBILITY='+visibility+' GIT_PROVIDER='+_app.git_provider+' SERVER_ID='+_app.server._id+' CALLBACK_URL='+AppConfig.AppDomain+'/v1/events ACTION=deploy APP_NAME='+_app.app_name+' APP_ID='+_app._id+ ' TEMPLATE='+_app.template+ ' NODE_VERSION='+_app.template_variation+' REPOSITORY="'+_app.app_repository+'" && /home/'+_app.server.superuser+'/.setup.sh >.app_'+_app.app_name+'.log.out 2>&1' },
				    ];
